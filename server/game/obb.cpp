...
OBB::OBB(float centerX, float centerY, float halfWidth, float halfHeight,
    ...
    : Entity(centerX, centerY, velocity),
      halfWidth(halfWidth),
      halfHeight(halfHeight),
      rotation(angle) {}

bool OBB::collidesWith(const OBB& other) const {
    Vector2 axes[4] = {Vector2({1, 0}, rotation), Vector2({0, 1}, rotation),
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // Proyección OBB A
        float projA = position * axis;
        float extentA = std::abs(axis * Vector2({halfWidth, 0}, rotation)) +
                        std::abs(axis * Vector2({0, halfHeight}, rotation));

        // Proyección OBB B
        float projB = other.position * axis;
        float extentB =
